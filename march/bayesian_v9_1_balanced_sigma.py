"""
Bayesian Hierarchical Analysis with Mixed Likelihood (v9.1)
領域別尤度: ポラリトン→透過率スペクトル形状, 共振器→スペクトル形状 + マルチピーク FWHM/位置

【v9.1 改善 — SIGMA_CAVITY_SPECTRUM を 0.025→0.05 に緩和してポラリトン/共振器バランス最適化】
★ 共振器領域スペクトル形状フィッティング追加 (最重要修正)
  - 共振器領域 (f ≥ CAVITY_LOWER) の全データ点を StudentT 尤度に追加
  - SIGMA_CAVITY_SPECTRUM で重みを制御 (ポラリトンより緩い制約)
  - eps_bg の制約がポラリトン + 共振器の両方から決定され、ピーク位置が改善
★ マルチピーク近傍マッチング (モード番号ミスマッチ修正)
  - 全キャビティピークを検出し近傍マッチング (max_distance = 半FSR)
  - 7T/9T で発生していた Δf ≈ 230 GHz (= 1 FSR) のミスマッチを解消
  - 10 datasets × ~3 peaks = ~30 ピーク制約 (v8 の ~3 倍)

【v8.1 基盤 — レビュー Issue #1-#4 修正】
★ キャビティピーク位置尤度 (Issue #1), FWHM フォールバック (Issue #2)
★ ベイズ因子計算の堅牢化 (Issue #3), 領域別定量指標 (Issue #4)

【v8.0 基盤機能 — ggg_research_strategy.pptx 解析方針実装】
★ 領域別複合尤度:
  - ポラリトン領域: p(D|Θ) ∝ T(ω, Θ, weight_pol) — スペクトル形状で推定
  - 高次共振器領域: p(D|Θ) ∝ T(ω, Θ, σ_cav) + FWHM + f_peak — スペクトル形状 + マルチピーク
...
# ポラリトン (σ~0.007) より緩く、背景 (σ~0.1) より厳しい中間値
SIGMA_CAVITY_SPECTRUM = 0.05
    ...
    for config in target_data_list:
        ...
        try:
            ...
            dataset = {
                ...
                'all_fwhms_obs':    all_fwhms_obs,   # list of all cavity peak FWHMs
                'all_peak_freqs_obs': all_peak_freqs_obs,  # list of all cavity peak freqs
            }
...
class MixedOutputModelOp(Op):
    ...
    def perform(self, _node, inputs, output_storage):
        ...
        for j in range(self.n_total_peaks):
            ds_idx = self.peak_dataset_indices[j]
            obs_freq = self.peak_obs_freqs_flat[j]
            obs_fwhm = self.peak_obs_fwhms_flat[j]

            matched = False
            if ds_idx in pred_peaks_cache:
                pred_fwhms, pred_freqs = pred_peaks_cache[ds_idx]
                if len(pred_freqs) > 0:
                    distances = [abs(pf - obs_freq) for pf in pred_freqs]
                    best_idx = np.argmin(distances)
                    if distances[best_idx] < PEAK_MATCH_MAX_DISTANCE:
                        peak_freq_pred_list.append(pred_freqs[best_idx])
                        fwhm_pred_list.append(pred_fwhms[best_idx])
                        matched = True

            if not matched:
                # ペナルティ: 観測値から大きくオフセット
                peak_freq_pred_list.append(obs_freq + 0.1)
                fwhm_pred_list.append(obs_fwhm * 2.0)
...
def _extract_lml_scalars(raw_values):
    """log_marginal_likelihood の値を再帰的にフラット化してスカラー配列にする。
    SMC の実装によって values が (chains, stages) の float 配列の場合と、
    要素がリスト (object dtype) の場合があるため両方に対応する。
    """
    flat = []
    def _recurse(obj):
...
def build_pymc_model(datasets, model_form, v6_params_H, v6_params_B):
    ...
    # 領域マスク（ポラリトン / 共振器スペクトル / 背景）
    polariton_masks = []
    cavity_spectrum_masks = []  # 【v9.0】共振器領域全体
    bg_masks        = []
    for d in datasets:
        weight = d['weight']
        polariton_masks.append(weight == 2.0)
        bg_masks.append(weight == 0.01)
        cavity_spectrum_masks.append(d['freq'] >= CAVITY_LOWER)
    ...
    cavity_spectrum_mask_concat = np.concatenate(cavity_spectrum_masks)  # 【v9.0】
    ...
    with model:
        ...
        # 8-a2. 【v9.0 新規】共振器領域スペクトル形状フィッティング
        # 共振器領域全体の透過スペクトルを直接フィット
        # → eps_bg がポラリトン+共振器の両方から制約され、ピーク位置が改善
        if np.any(cavity_spectrum_mask_concat):
            obs_cav_spec  = trans_obs_concat[cavity_spectrum_mask_concat]
            pred_cav_spec = trans_pred_concat[cavity_spectrum_mask_concat]
            sigma_cav_spec = np.full_like(obs_cav_spec, SIGMA_CAVITY_SPECTRUM)
            dist_cav_spec = pm.StudentT.dist(nu=NU_STUDENTT, mu=pred_cav_spec, sigma=sigma_cav_spec)
            ll_cav_spec   = pm.logp(dist_cav_spec, obs_cav_spec)
            pm.Potential('ll_cavity_spectrum', ll_cav_spec.sum())
...
def main():
    ...
    eval_results = {
        ...
        'sigma_cavity_spectrum': SIGMA_CAVITY_SPECTRUM,
    ...
    print(f"  尤度      : {LIKELIHOOD_TYPE} (ポラリトン=スペクトル / 共振器=スペクトル+マルチピーク)")
