...
class MixedOutputModelOp(Op):
    ...
    def perform(self, _node, inputs, output_storage):
        ...
        # 【v10.1】マルチピーク近傍マッチング (改善版: フォールバック強化)
        fwhm_pred_list = []
        peak_freq_pred_list = []
        used_pred_peak_indices = {}  # dataset_idx -> set(pred_peak_idx)
        ...
        for j in range(self.n_total_peaks):
            ...
            if ds_idx in pred_peaks_cache:
                ...
                if len(pred_freqs) > 0:
                    used_set = used_pred_peak_indices.setdefault(ds_idx, set())
                    candidate_indices = [i for i in range(len(pred_freqs)) if i not in used_set]
                    if len(candidate_indices) == 0:
                        candidate_indices = list(range(len(pred_freqs)))

                    distances = [abs(pred_freqs[i] - obs_freq) for i in candidate_indices]
                    best_local_idx = int(np.argmin(distances))
                    best_idx = candidate_indices[best_local_idx]

                    # v10.1: 距離閾値超過でも最近傍ピークを使用して連続性を維持する
                    peak_freq_pred = float(pred_freqs[best_idx])
                    fwhm_pred = float(pred_fwhms[best_idx]) if best_idx < len(pred_fwhms) else float(obs_fwhm)

                    peak_freq_pred_list.append(peak_freq_pred)
                    fwhm_pred_list.append(fwhm_pred)
                    used_set.add(best_idx)
                    matched = True

            if not matched:
                # ピークが全く検出されなかった場合のみペナルティ
                peak_freq_pred_list.append(obs_freq + 0.05)
...
def plot_posterior_predictive_spectra(trace, datasets, model_form='H', save_dir=None, n_samples=300):
    ...
    for idx, data in enumerate(datasets):
        ...
        rmse_full = np.sqrt(np.mean((trans_obs - trans_median) ** 2))

        # キャビティ ΔFWHM [GHz]（主指標: 振幅スケールの影響を受けない）
        delta_fwhm_ghz = abs(fwhm_obs - fwhm_pred) * 1000 if (fwhm_obs and fwhm_pred) else np.nan
        ...
        fwhm_str = (f"FWHM obs={fwhm_obs*1000:.1f} pred={fwhm_pred*1000:.1f} "
                    f"ΔFWHM={delta_fwhm_ghz:.1f} GHz"
                    if fwhm_obs and fwhm_pred else "")
        peak_str = f" Δf={peak_err_ghz:.1f}GHz" if not np.isnan(peak_err_ghz) else ""
        ax.set_title(
            f"{label}  RMSE_pol={rmse_pol:.4f} (full={rmse_full:.4f}){peak_str}\n{fwhm_str}",
...
def plot_posterior_predictive_spectra_combined(trace_H, trace_B, datasets, save_dir=None, n_samples=200):
    ...
    for idx, data in enumerate(datasets):
        ...
        pol_mask = freq <= POLARITON_UPPER
        rmse_H_pol  = np.sqrt(np.mean((trans_obs[pol_mask] - median_H[pol_mask]) ** 2)) if np.any(pol_mask) else np.nan
        rmse_B_pol  = np.sqrt(np.mean((trans_obs[pol_mask] - median_B[pol_mask]) ** 2)) if np.any(pol_mask) else np.nan
        rmse_H_full = np.sqrt(np.mean((trans_obs - median_H) ** 2))
        rmse_B_full = np.sqrt(np.mean((trans_obs - median_B) ** 2))

        fwhm_obs_val = data.get('fwhm_obs')
        fwhm_H, _ = compute_cavity_peak_info(freq, median_H)
        fwhm_B, _ = compute_cavity_peak_info(freq, median_B)
        dfwhm_H = f"{abs(fwhm_obs_val - fwhm_H)*1000:.1f}" if (fwhm_obs_val and fwhm_H) else "N/A"
        dfwhm_B = f"{abs(fwhm_obs_val - fwhm_B)*1000:.1f}" if (fwhm_obs_val and fwhm_B) else "N/A"

        ax.set_title(
            f"{label}  RMSE_pol: H={rmse_H_pol:.4f} B={rmse_B_pol:.4f}"
            f"  (full: H={rmse_H_full:.4f} B={rmse_B_full:.4f})\n"
            f"ΔFWHM[GHz]: H={dfwhm_H} B={dfwhm_B}",
