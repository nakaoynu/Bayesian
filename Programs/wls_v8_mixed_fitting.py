"""
Weighted Least Squares Mixed Fitting (v8.3-WLS)
領域別残差: ポラリトン→透過率スペクトル形状+ピーク特徴量, 共振器→FWHM + ピーク位置

【v8.3 変更点 (8T/9T ポラリトン領域フィッティング改善)】
  - B依存結合パラメータ a_B を追加: a_eff = a + a_B × (B − B_REF)
    → 高磁場でのポラリトン分裂量を独立に調整可能に
  - ポラリトン領域のピーク特徴量残差を追加:
    → ピーク位置 + ピーク高さを明示的にフィッティング対象に
  - (v8.2 から継承) ポラリトン領域全域方式 (f < POLARITON_UPPER)

【混合残差構造】
  - ポラリトン領域 (f < 0.361505 THz):
      全域の透過率スペクトル形状残差 (重み 2.0)
      + ピーク位置・ピーク高さ特徴量残差
...
【パラメータ (14個)】
  g, a, B4, B6, eps_bg, gamma_1 ... gamma_7, gamma_1_B, a_B

【継承元】bayesian_v8_mixed_likelihood.py v8.1 → v8.2 → v8.3
...
CAVITY_FEATURE_WEIGHT = 5.0  # FWHM・ピーク位置残差の重み倍率
POLARITON_FEATURE_WEIGHT = 5.0  # ポラリトンピーク特徴量残差の重み倍率
SIGMA_POL_PEAK_FREQ   = 0.005   # THz (5 GHz) — ポラリトンピーク位置残差の正規化用
SIGMA_POL_PEAK_HEIGHT = 0.05    # ポラリトンピーク高さ残差の正規化用 (正規化透過率)
N_MONTE_CARLO = 500        # パラメータ不確かさ伝播用モンテカルロサンプル数

# gamma 正則化 (ベイズ階層事前分布に相当する L2 ペナルティ)
LAMBDA_GAMMA = 1.0         # gamma 正則化の強度
LOG_GAMMA_PRIOR_MEAN = -3.5  # log(gamma) の事前平均 (≈ 0.030 THz)
LOG_GAMMA_PRIOR_SD   = 0.5   # log(gamma) の事前標準偏差

# B4/B6 正則化 (結晶場パラメータの事前分布)
LAMBDA_CF = 30.0           # 結晶場正則化の強度
B4_PRIOR  = 0.02           # B4 事前平均 (K) — v6 全形式の平均近似
B6_PRIOR  = -0.001         # B6 事前平均 (K)
SIGMA_B4  = 0.04           # B4 事前標準偏差 (K)
SIGMA_B6  = 0.015          # B6 事前標準偏差 (K)
...
PARAM_NAMES = ['g', 'a', 'B4', 'B6', 'eps_bg',
               ...
               'gamma_5', 'gamma_6', 'gamma_7', 'gamma_1_B', 'a_B']

B_REF = 6.5  # gamma_1 B依存の参照磁場 (T)
...
PARAM_BOUNDS_LOWER = np.array([
    ...
    -0.02,  # gamma_1_B (THz/T) — gamma_1のB依存係数
    -3.0,   # a_B — 結合パラメータのB依存係数
])
...
def get_effective_gamma(gamma_array, B, gamma_1_B=0.0):
    """B依存gamma_1を計算して有効gamma配列を返す"""
    gamma_eff = gamma_array.copy()
    gamma_eff[0] = np.clip(gamma_array[0] + gamma_1_B * (B - B_REF), 0.005, 0.5)
    return gamma_eff


def get_effective_a(a, a_B, B):
    """B依存結合パラメータを計算して有効aを返す"""
    return max(a + a_B * (B - B_REF), 0.1)
...
# ============================================================================
def compute_cavity_peak_info(freq, trans, cavity_lower=CAVITY_LOWER, target_freq=None):
    """共振器領域の最も顕著なピークの FWHM とピーク周波数を返す。
    target_freq が指定された場合、最も近いピークを優先的に選択する。"""
    ...
    if target_freq is not None and len(peaks) > 1:
        # 最近接ピークマッチング: target_freq に最も近いピークを選択
        distances = np.abs(freq_cav[peaks] - target_freq)
    ...
    # 放物線補間で sub-grid ピーク位置を算出（勾配ベース最適化に必須）
    if 0 < peak_idx < len(trans_cav) - 1:
        y_m = trans_cav[peak_idx - 1]
        y_0 = trans_cav[peak_idx]
        y_p = trans_cav[peak_idx + 1]
        denom_parab = y_m - 2.0 * y_0 + y_p
        if abs(denom_parab) > 1e-15:
            delta = 0.5 * (y_m - y_p) / denom_parab
            peak_freq = freq_cav[peak_idx] + np.clip(delta, -0.5, 0.5) * df
        else:
            peak_freq = freq_cav[peak_idx]
    else:
        peak_freq = freq_cav[peak_idx]

    return float(widths_samples[0] * df), float(peak_freq)
...
def compute_polariton_peak_info(freq, trans, polariton_upper=POLARITON_UPPER):
    """ポラリトン領域のピーク情報 (最大2ピーク) を返す。

    Returns: list of dict with 'freq' and 'height' keys, sorted by frequency.
    """
    df = freq[1] - freq[0] if len(freq) > 1 else 1.0
    mask = freq <= polariton_upper
    if not np.any(mask):
        return []
    freq_pol = freq[mask]
    trans_pol = trans[mask]
    peaks, props = find_peaks(trans_pol, prominence=0.02, width=2)
    if len(peaks) == 0:
        return []
    # Sort by frequency
    sort_idx = np.argsort(freq_pol[peaks])
    sorted_peaks = peaks[sort_idx]
    sorted_proms = props['prominences'][sort_idx]
    # Keep top 2 by prominence
    if len(sorted_peaks) > 2:
        top2_idx = np.argsort(sorted_proms)[-2:]
        sorted_peaks = np.sort(sorted_peaks[top2_idx])
    result = []
    for pk in sorted_peaks:
        # Sub-grid peak position (parabolic interpolation)
        if 0 < pk < len(trans_pol) - 1:
            y_m, y_0, y_p = trans_pol[pk - 1], trans_pol[pk], trans_pol[pk + 1]
            denom_parab = y_m - 2.0 * y_0 + y_p
            if abs(denom_parab) > 1e-15:
                delta = 0.5 * (y_m - y_p) / denom_parab
                peak_freq = freq_pol[pk] + np.clip(delta, -0.5, 0.5) * df
            else:
                peak_freq = freq_pol[pk]
        else:
            peak_freq = freq_pol[pk]
        result.append({'freq': float(peak_freq), 'height': float(trans_pol[pk])})
    return result
...
def detect_peaks_and_classify(freq, trans, polariton_upper=POLARITON_UPPER, cavity_lower=CAVITY_LOWER):
    ...
    v8.2: ポラリトン領域は f < polariton_upper の全域とする（ピーク検出に依存しない）。
    これにより f≈0 付近のピークや幅広い構造も確実に捕捉される。
    共振器領域は従来通りピーク検出ベースで決定する。
    """
    # ポラリトン領域: POLARITON_UPPER 以下の全域
    polariton_regions = []
    if freq[0] < polariton_upper:
        polariton_regions = [(float(freq[0]), float(min(freq[-1], polariton_upper)))]
    ...
    for pf, pw in zip(peak_freqs, peak_widths_arr):
        if pf >= cavity_lower:
            f_start = max(cavity_lower, pf - 1.5 * pw)
            f_end   = min(freq[-1], pf + 1.5 * pw)
            if f_end > f_start:
                cavity_regions.append((f_start, f_end))
    return polariton_regions, cavity_regions
...
def load_all_datasets(target_data_list):
    ...
    for config in target_data_list:
        ...
        try:
            ...
            pol_peaks_obs = compute_polariton_peak_info(freq, trans)
            ...
            dataset = {
                ...
                'pol_peaks_obs':    pol_peaks_obs,
            }
            datasets.append(dataset)

            fwhm_str = f"{fwhm_obs*1000:.1f} GHz" if fwhm_obs is not None else "N/A"
            pol_str = f"{len(pol_peaks_obs)} pol peaks" if pol_peaks_obs else "no pol peaks"
            print(f"✓ {label} (B={config['B']}T, T={config['T']}K): {len(freq)} points, "
                  f"cav FWHM={fwhm_str}, {pol_str}")
...
def v6_to_param_vector(v6_params):
    """v6 パラメータ辞書を 14 要素のベクトルに変換 (gamma_1_B, a_B 含む)"""
...
def compute_mixed_residuals(params, datasets, model_form):
    """
    パラメータ params (14要素) から全データセットの混合残差ベクトルを構築する。

    残差構成:
    1. ポラリトン領域: 形状残差 (weight == 2.0)
    2. ポラリトン特徴量: ピーク位置 + ピーク高さ (v8.3 追加)
    3. 共振器領域:     スペクトル形状残差 (weight == 1.0)
    4. FWHM:          CAVITY_FEATURE_WEIGHT * (fwhm_pred - fwhm_obs) / sigma_fwhm
    5. ピーク位置:     CAVITY_FEATURE_WEIGHT * (peak_freq_pred - peak_freq_obs) / sigma_peak_freq
    6. 背景領域:       (trans_pred - trans_obs) * sqrt(weight) / sigma
    """
    g       = params[0]
    a       = params[1]
    B4      = params[2]
    ...
    gamma_1_B   = params[12] if len(params) > 12 else 0.0
    a_B         = params[13] if len(params) > 13 else 0.0
    ...
    for data in datasets:
        ...
        # B依存パラメータを適用
        gamma_eff = get_effective_gamma(gamma_array, B_ext, gamma_1_B)
        a_eff     = get_effective_a(a, a_B, B_ext)

        # 透過率スペクトル計算
        trans_pred = calculate_transmission_for_params(
            freq, B_ext, T_val, g, a_eff, B4, B6, eps_bg, gamma_eff, model_form)
        ...
        # ポラリトン特徴量残差 (ピーク位置 + ピーク高さ)
        pol_peaks_obs = data.get('pol_peaks_obs', [])
        if len(pol_peaks_obs) >= 1:
            pol_peaks_pred = compute_polariton_peak_info(freq, trans_pred)
            if len(pol_peaks_pred) >= len(pol_peaks_obs):
                # 近接マッチング: 各観測ピークに最も近い予測ピークを割り当て
                used = set()
                for obs_pk in pol_peaks_obs:
                    best_j, best_dist = None, float('inf')
                    for j, pred_pk in enumerate(pol_peaks_pred):
                        if j in used:
                            continue
                        dist = abs(pred_pk['freq'] - obs_pk['freq'])
                        if dist < best_dist:
                            best_dist, best_j = dist, j
                    if best_j is not None:
                        used.add(best_j)
                        pred_pk = pol_peaks_pred[best_j]
                        r_pos = POLARITON_FEATURE_WEIGHT * (pred_pk['freq'] - obs_pk['freq']) / SIGMA_POL_PEAK_FREQ
                        r_height = POLARITON_FEATURE_WEIGHT * (pred_pk['height'] - obs_pk['height']) / SIGMA_POL_PEAK_HEIGHT
                        all_residuals.append(np.array([r_pos, r_height]))
            else:
                # 予測ピーク不足 → ペナルティ
                n_missing = len(pol_peaks_obs) - len(pol_peaks_pred)
                penalty = POLARITON_FEATURE_WEIGHT * 2.0
                all_residuals.append(np.full(n_missing * 2, penalty))

        # 共振器領域 (weight == 1.0) のスペクトル残差
        cav_mask = weight == 1.0
        if np.any(cav_mask):
            sigma_cav = 0.01 / np.sqrt(weight[cav_mask])
            residual_cav = (trans_pred[cav_mask] - trans_obs[cav_mask]) / sigma_cav
            all_residuals.append(residual_cav)
        ...
        # 共振器領域: FWHM + ピーク位置残差 (重み増強)
        fwhm_obs      = data['fwhm_obs']
        peak_freq_obs = data['peak_freq_obs']

        if fwhm_obs is not None:
            fwhm_pred, peak_freq_pred = compute_cavity_peak_info(
                freq, trans_pred, target_freq=peak_freq_obs)
            if fwhm_pred is None:
                fwhm_pred = fwhm_obs * 2.0
                peak_freq_pred = peak_freq_obs + 0.1

            residual_fwhm = CAVITY_FEATURE_WEIGHT * (fwhm_pred - fwhm_obs) / SIGMA_FWHM
            all_residuals.append(np.array([residual_fwhm]))

            if peak_freq_obs is not None:
                residual_peak = CAVITY_FEATURE_WEIGHT * (peak_freq_pred - peak_freq_obs) / SIGMA_PEAK_FREQ
                all_residuals.append(np.array([residual_peak]))

    # gamma L2 正則化ペナルティ (ベイズ階層事前分布に相当)
    gamma_arr = np.array(params[5:12], dtype=float)
    log_gamma = np.log(np.clip(gamma_arr, 1e-6, None))
    gamma_penalty = LAMBDA_GAMMA * (log_gamma - LOG_GAMMA_PRIOR_MEAN) / LOG_GAMMA_PRIOR_SD
    all_residuals.append(gamma_penalty)

    # B4/B6 正則化ペナルティ
    cf_penalty = LAMBDA_CF * np.array([
        (B4 - B4_PRIOR) / SIGMA_B4,
        (B6 - B6_PRIOR) / SIGMA_B6,
    ])
    all_residuals.append(cf_penalty)
...
def estimate_parameter_uncertainty(result):
    ...
    chi2_reduced = 2.0 * result.cost / max(n_residuals - n_params, 1)
    print(f"  chi2_reduced = {chi2_reduced:.4f}")
    if chi2_reduced > 10.0:
        print(f"  ⚠️ chi2_reduced が大きい ({chi2_reduced:.1f} >> 1): sigma が過小か、モデル不適合の可能性")

    # 共分散行列: C = s² * (J^T J)^{-1}
    try:
        JtJ = J.T @ J
        cov_matrix = chi2_reduced * np.linalg.inv(JtJ)
        ...
        for i, name in enumerate(PARAM_NAMES):
            val = abs(result.x[i])
            if val > 1e-10 and param_std[i] / val > 1.0:
                print(f"  ⚠️ {name}: 相対不確実性 {param_std[i]/val*100:.0f}% (値={result.x[i]:.6f}, std={param_std[i]:.6f}) — 制約不良")

        # 高相関ペアの警告
        for i in range(n_params):
            for j in range(i+1, n_params):
                if abs(correlation_matrix[i, j]) > 0.9:
                    print(f"  ⚠️ 高相関: {PARAM_NAMES[i]} / {PARAM_NAMES[j]} = {correlation_matrix[i,j]:.3f}")
...
def plot_fit_spectra(params, datasets, model_form, save_dir=None,
    ...
    fig.suptitle(f'WLS Fit Spectra ({model_form}-form) — v8.3-WLS Mixed Residual',
                 fontsize=12, y=0.995)
    axes = axes.flatten()
    ...
    for idx, data in enumerate(datasets):
        ...
        # B依存パラメータ
        gamma_eff = get_effective_gamma(gamma_array, B, gamma_1_B)
        a_eff     = get_effective_a(a, a_B, B)

        # 最適フィット
        trans_fit = calculate_transmission_for_params(
            freq, B, T, g, a_eff, B4, B6, eps_bg, gamma_eff, model_form)
        ...
        if mc_params is not None:
            ...
            for i in range(n_mc):
                p = mc_params[i]
                g1B_mc = p[12] if len(p) > 12 else 0.0
                aB_mc  = p[13] if len(p) > 13 else 0.0
                gamma_eff_mc = get_effective_gamma(p[5:12], B, g1B_mc)
                a_eff_mc     = get_effective_a(p[1], aB_mc, B)
                trans_mc[i] = calculate_transmission_for_params(
                    freq, B, T, p[0], a_eff_mc, p[2], p[3], p[4], gamma_eff_mc, model_form)
            ci_lo = np.percentile(trans_mc, 2.5, axis=0)
            ci_hi = np.percentile(trans_mc, 97.5, axis=0)
            has_ci = True

        fwhm_pred, peak_freq_pred = compute_cavity_peak_info(
            freq, trans_fit, target_freq=peak_freq_obs)
...
def plot_parameter_summary(params, param_std, model_form, save_dir=None):
    ...
    fig.suptitle(f'Parameter Estimates with 1σ Uncertainty ({model_form}-form) — v8.3-WLS',
                 fontsize=14)
    axes = axes.flatten()
    ...
    if len(params) > 12:
        ax = axes[12]
        val = params[12]
        err = param_std[12] if np.isfinite(param_std[12]) else 0
        ax.barh(['γ₁_B'], [val], xerr=[err], color='coral', edgecolor='black',
                capsize=5, height=0.5)
        ax.set_xlabel('γ₁_B (THz/T)', fontsize=9)
        ax.set_title(f'γ₁_B = {val:.4f} ± {err:.4f}', fontsize=9, fontweight='bold')
        ax.grid(alpha=0.3, axis='x')
    ...
    if len(params) > 13:
        ax = axes[13]
        val = params[13]
        err = param_std[13] if np.isfinite(param_std[13]) else 0
        ax.barh(['a_B'], [val], xerr=[err], color='coral', edgecolor='black',
                capsize=5, height=0.5)
        ax.set_xlabel('a_B (1/T)', fontsize=9)
        ax.set_title(f'a_B = {val:.4f} ± {err:.4f}', fontsize=9, fontweight='bold')
        ax.grid(alpha=0.3, axis='x')

    # Hide unused axes
    for i in range(max(14, len(params)), len(axes)):
...
def plot_correlation_matrix(correlation_matrix, model_form, save_dir=None):
    ...
    ax.set_title(f'Parameter Correlation Matrix ({model_form}-form) — v8.3-WLS', fontsize=12)
    plt.tight_layout()
    if save_dir:
...
def plot_susceptibility(params, datasets, model_form, save_dir=None):
    ...
    for idx, data in enumerate(datasets):
        ...
        a_eff   = get_effective_a(a, a_B, B_ext)
        G0      = a_eff * mu0 * N_SPIN * (g * muB) ** 2 / (2 * hbar) / THZ_TO_RAD_S
        chi     = G0 * chi_raw
...
def plot_residual_analysis(params, datasets, model_form, save_dir=None):
    ...
    nrows = (n_datasets + 1) // 2
    fig, axes = plt.subplots(nrows, ncols, figsize=(14, 3.5 * nrows))
    fig.suptitle(f'Residual Analysis ({model_form}-form) — v8.3-WLS', fontsize=12, y=0.995)
    axes = axes.flatten()
    ...
    for idx, data in enumerate(datasets):
        ...
        gamma_eff = get_effective_gamma(gamma_array, data['B'], gamma_1_B)
        a_eff     = get_effective_a(a, a_B, data['B'])
        trans_fit = calculate_transmission_for_params(
            freq, data['B'], data['T'], g, a_eff, B4, B6, eps_bg, gamma_eff, model_form)
...
def main():
    ...
    print(f"\n{'='*80}")
    print("WLS Analysis v8.3 — Mixed Residual + Polariton Features (a_B + peak pos/height)")
    print(f"v8.3: B依存結合 a_B, ポラリトンピーク特徴量残差追加")
    print(f"ポラリトン全域方式 (f < {POLARITON_UPPER} THz)")
    print(f"{'='*80}")
    ...
    for form, result, std, corr in [('H', result_H, std_H, corr_H),
        ...
        params_out['gamma'] = [float(x) for x in result.x[5:12]]
        ...
        gamma_1_B_val = result.x[12] if len(result.x) > 12 else 0.0
        a_B_val       = result.x[13] if len(result.x) > 13 else 0.0
        for data in datasets:
            freq = data['freq']
            trans_obs = data['trans']
            gamma_eff = get_effective_gamma(gamma_array, data['B'], gamma_1_B_val)
            a_eff     = get_effective_a(a, a_B_val, data['B'])
            trans_fit = calculate_transmission_for_params(
                freq, data['B'], data['T'], g, a_eff, B4, B6, eps_bg, gamma_eff, form)
